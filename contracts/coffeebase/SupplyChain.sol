...
contract SupplyChain is FarmerRole, DistributorRole, RetailerRole, ConsumerRole, Ownable {
  ...
  // Create list of unique upc
  mapping (uint => bool) uniqueItems;
 ...
 onlyFarmer public 
  {
    ...
    // Increment sku
    sku = sku + 1;
     uniqueItems[_upc] = true;
    // Emit the appropriate event
   
    emit Harvested(item.upc);
  }

  function checkIfUpcExist(uint _upc) public view returns (bool) {
    return uniqueItems[_upc];
  }
  ...
  function buyItem(uint _upc) onlyDistributor public payable 
    ...
    items[_upc].distributorID = items[_upc].ownerID = distributor;
  ...
  function purchaseItem(uint _upc) onlyConsumer public 
    ...
    items[_upc].consumerID = items[_upc].ownerID = consumer;
